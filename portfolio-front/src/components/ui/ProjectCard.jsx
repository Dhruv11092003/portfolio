...
const ProjectCard = ({ title, tech, description, links = [] }) => {
  ...
  return (
    <motion.article whileHover={{ y: -8, scale: 1.01 }} className="bento-card h-full">
      <h3 className="font-display text-xl font-bold">{title}</h3>
      <div className="mt-3 flex flex-wrap gap-2">
        {tags.map((tag) => (
          <span key={tag} className="rounded-full bg-cyan-500/15 px-2.5 py-1 text-[11px] font-mono text-cyan-700 dark:text-cyan-300">
            {tag}
          </span>
        ))}
      </div>
